...
class ProyectoController extends Controller
{
    ...
    public function listadocumentosAction($idproyecto)
    {
        $em = $this->getDoctrine()->getManager();
        $dql = "select x from ProyectoBundle:Documento x where x.proyectoId= :idproyecto";
        $query = $em->createQuery($dql);
        $query->setParameter('idproyecto',$idproyecto);
        $docupro = $query->getResult();
        
        $dql = "select x from ProyectoBundle:Documentoactividad x join x.actividadId a join a.tarea t join t.proyecto p where p.id= :idproyecto";
        $query = $em->createQuery($dql);
        $query->setParameter('idproyecto',$idproyecto);
        $docutar = $query->getResult();
        
        $proyecto = $em->getRepository('ProyectoBundle:Proyecto')->find($idproyecto); 
        

        return $this->render('ProyectoBundle:Proyecto:documentoproyecto.html.twig', array(
            'entitiesp' => $docupro,
            'entitiest' => $docutar,
            'proyecto'=>$proyecto
        ));
