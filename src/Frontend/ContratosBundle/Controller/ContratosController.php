...
class ContratosController extends Controller
{
    ...
    public function createAction(Request $request)
    {
        ...
        $anio_actual =  date("Y");
        $anio =  date("y");
        $desde = $anio_actual."-01-01";
        $hasta = $anio_actual."-12-31";

        $entity  = new Contratos();
        $form = $this->createForm(new ContratosType(), $entity);
        $form->bind($request);

        $dql   = "SELECT cont FROM ContratosBundle:Contratos cont where cont.fechaRegistro between :desde and :hasta";
        $query = $em->createQuery($dql)->setParameters(array('desde'=>$desde, 
                                                            'hasta'=>$hasta));
        ...
        if($cont < 10 )
        {
            $codigo = "CJ/0".$cont."/".$anio."/".$direccion;
        }else
        {
            $codigo = "CJ/".$cont."/".$anio."/".$direccion;
        }
        ...
        if ($form->isValid()) {
            ...
            if($form['file']->getData())
            {
                ...
                //GUARDO EL ARCHIVO
                if($file->move('/var/www/uploads/contratos/',$nombre.'_'.\date("Gis").'.'.$extension) )
                {
    ...
    public function deleteAction($id)
    {
        ...
        unlink('/var/www/uploads/contratos/'.$nombre.'');
