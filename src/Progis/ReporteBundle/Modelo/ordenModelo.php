...
class ordenModelo
{
    ...
    public function ordenPriometa($recibe){
        ...
        foreach ($ids as $id) {
            ...
            $actividad= $em->getRepository('ProyectoBundle:ProcesarActividad')->find($id);
            $orden[]=$id."-".$cont;
            
            if($actividad->getTipotiempo()==2)
                $horas=$actividad->getTiempoestimado();
            elseif($actividad->getTipotiempo()==3)
                $minutos=$actividad->getTiempoestimado();
