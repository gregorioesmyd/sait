...
class ubicacionModelo
{
    ...
    function __construct($em) {
        $this->em = $em;
        $this->respuesta=array('error'=>null,'sms'=>null,'numero'=>null,'tiempo'=>null);
    }
    ...
    public function proceso($desde,$idusuario,$id,$bundleActual,$bitacoraModelo) {
        ...
        if($bundleActual=='ticket'):
            $p = $em->getRepository('TicketBundle:ProcesarTicket')->find($id);
            $idResponsable=$p->getResponsable()->getUsuario()->getId();
        elseif($bundleActual=='proyecto'):
            $p = $em->getRepository('ProyectoBundle:ProcesarActividad')->find($id);
            $idResponsable=$p->getResponsable()->getMiembroespacio()->getUsuario()->getId();
        endif;
        ...
        //valido que no se coloquen dos tickets en proceso
            ...
            $query->setParameter('idusuario',$idResponsable);
        ...
        if(!empty($tarjetaAsignadaTicket)):
            ...
            $respuesta['sms']='Ya posee un ticket en proceso, puede visualizarlo <a target="__blank" href="/sait/web/app.php/progis/ticket/procesarTicket">aquí</a>';
            return $respuesta;
        elseif(!empty($tarjetaAsignadaProyecto)):
            $respuesta['error']=true;
            $respuesta['sms']='Ya posee una actividad en proceso, puede visualizarla <a target="__blank" href="/sait/web/app.php/progis/proyecto/procesarActividad/'.$tarjetaAsignadaProyecto[0]->getObjetivo()->getId().'">aquí</a>.';
            return $respuesta;
        else:
    ...
    public function culminado($desde,$idusuario,$id,$bundleActual,$bitacoraModelo,$proyectoModelo,$metaModelo) {
        //inicializo respuestas
        $respuesta=  $this->respuesta;
        $em=$this->em;
        ...
        if($p->getTiemporeal()!=null):
            ...
            //actualizo metas cuando una actividad es culminada y la actividad esta como meta
            if($bundleActual=='proyecto'):
                if($p->getUbicacionPriometa()==2):
                    $metaActividad = $em->getRepository('PrincipalBundle:MetaActividad')->findByActividad($p);
                    $metaModelo->estatus(array(0=>$metaActividad[0]->getMeta()));
    ...
    public function ubicacion($recibe,$ubicacion,$idusuario,$objetivoModelo,$bitacoraModelo,$admin,$proyectoModelo,$metaModelo,$trabajoRealizadoModelo){
        $em=$this->em;
        $respuesta=  $this->respuesta;
        ...
        if($bundleActual=='ticket'):
            ...
            if($desde=='culminado' and $p->getTicket()->getEstatus()==4 or $hasta=='culminado' and $p->getUbicacion()==4):
                $respuesta['error']=true;
        ...
        //redirijo hasta la columna donde mueve la tarjeta
            ...
            $respuesta=$this->$hasta($desde,$idusuario,$id,$bundleActual,$bitacoraModelo,$proyectoModelo,$metaModelo);
        endif;
        ...
        //guardo la ubicacion si no retorna un error
            ...
            if($bundleActual=='proyecto'):
                $objetivoModelo->estatusobjetivo($p->getObjetivo()->getId(),$proyectoModelo);
                $objetivoModelo->porcentajeobjetivo($p->getObjetivo()->getId(),$proyectoModelo);
                if($desde=='proceso'):
                    $ticket = $em->getRepository('ProyectoBundle:ProcesarActividad')->find($id); 
                    $trabajoRealizadoModelo->guardaActividad($p);
                endif;
            elseif($bundleActual=='ticket' and $desde=='proceso'):
                $ticket = $em->getRepository('TicketBundle:ProcesarTicket')->find($id);
                $trabajoRealizadoModelo->guardaTicket($p);
            endif;
    ...
    public function ubicacionPriometa($recibe,$ubicacion,$idusuario){
        ...
        elseif($tipotarjeta=='proyecto'):
            ...
            if($hasta=='priometa'):
                ...
                //sumo el tiempo de actividades ya asignadas
                    $metAct = $em->getRepository('PrincipalBundle:MetaActividad')->findByMeta($idmeta);
                    $horas=0;$minutos=0;$tiempo=0;
                    foreach ($metAct as $ma) {
                        if($ma->getActividad()->getTipotiempo()==2)
                            $horas +=$ma->getActividad()->getTiempoestimado();
                        elseif($ma->getActividad()->getTipotiempo()==3)
                            $minutos +=$ma->getActividad()->getTiempoestimado();
                    }
                    $tiempoAsignado =$horas+($minutos/60);

                //sumo el tiempo ya asignado con el que se va a asignar para ver si supera el total de la meta
                    if($p->getTipotiempo()==2)
                        $tiempoAsignado +=$p->getTiempoestimado();
                    elseif($p->getTipotiempo()==3)
                        $tiempoAsignado +=($p->getTiempoestimado()/60);

                    if($tiempoAsignado>$m->getHoras()){
                        $respuesta['error']=true;
                        $respuesta['sms']='La cantidad de horas asignadas no puede superar las horas totales de la meta.';
                        return $respuesta;
                    }
                //fin de vaidacion
            ...
            //sumo tiempo total para mostrar
            $metAct = $em->getRepository('PrincipalBundle:MetaActividad')->findByMeta($idmeta);
            $horas=0;$minutos=0;$tiempo=0;
            foreach ($metAct as $ma) {
                if($ma->getActividad()->getTipotiempo()==2)
            ...
            $respuesta['numero']=$p->getOrdenPriometa();
            $respuesta['sms']="Tarjeta ubicada con exito.";
            $respuesta['tiempo']=  number_format($tiempo,2);
